...
{
    UIView *fromView = nil;
    if ([_delegate respondsToSelector:@selector(getCurrentThumbViewWithPage:)]) {
        fromView = [_delegate getCurrentThumbViewWithPage:curItem];
    }
    [self presentFromImageView:fromView toContainer:container animated:animated completion:completion];
}
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    if (!fromView) {
        if ([_delegate respondsToSelector:@selector(getCurrentThumbViewWithPage:)]) {
            fromView = [_delegate getCurrentThumbViewWithPage:currentPage];
        }
    }
