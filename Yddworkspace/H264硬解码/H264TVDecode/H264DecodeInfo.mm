...
int SeValue(unsigned char *pBuff, unsigned int nLen, unsigned int &nStartBit)
{
...
bool h264_decode_sps(unsigned char * buf,unsigned int nLen,int &width,int &height,int &fps)
{
  ...
  if(nal_unit_type==7)
  {
    ...
    else if( pic_order_cnt_type == 1 )
    {
      ...
      int offset_for_non_ref_pic=SeValue(buf,nLen,StartBit);
      int offset_for_top_to_bottom_field=SeValue(buf,nLen,StartBit);
      int num_ref_frames_in_pic_order_cnt_cycle=Ue(buf,nLen,StartBit);
      
      int *offset_for_ref_frame=new int[num_ref_frames_in_pic_order_cnt_cycle];
      for( int i = 0; i < num_ref_frames_in_pic_order_cnt_cycle; i++ )
        offset_for_ref_frame[i]=SeValue(buf,nLen,StartBit);
