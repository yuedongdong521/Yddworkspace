...
static ScreenRecorder *_recoder;
...
//    CGSize size = [[UIScreen mainScreen] bounds].size;
    ...
    self.videoPath = [documents stringByAppendingPathComponent:@"YUVVideo.mp4"];
...
+(instancetype)shareRecorder
{
  ...
  dispatch_once(&onceToken, ^{
    _recoder = [[ScreenRecorder alloc] init];
  });
  return _recoder;
}
...
- (void)writerVideoBuffer:(CVPixelBufferRef)buffer
{
  if (!self.isRecording) {
    return;
  }
  float millisElapsed = [[NSDate date] timeIntervalSinceDate:_startedAt] * 1000.0;
  CMTime frameTime = CMTimeMake((int)millisElapsed, 1000);
  [self appendVideoFrameAtTime:frameTime buffer:buffer];
}
- (void)appendVideoFrameAtTime:(CMTime)frameTime buffer:(CVPixelBufferRef)buffer
{
  if (![self.videoWriterInput isReadyForMoreMediaData]) {
    NSLog(@"Not ready for video data");
  } else {
