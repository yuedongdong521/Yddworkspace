...
- (void)stopGIFView:(UIImageView*)view {
  CGImageSourceRef ref = (__bridge CGImageSourceRef)(
      [[PlayGIFManager shared].gifSourceRefMapTable objectForKey:view]);
  if (ref) {
...
- (void)setGifPath:(NSString*)gifPath {
  objc_setAssociatedObject(self, kGifPathKey, gifPath,
...
- (void)setGifData:(NSData*)gifData {
  objc_setAssociatedObject(self, kGifDataKey, gifData,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (NSNumber*)index {
...
- (void)setIndex:(NSNumber*)index {
  objc_setAssociatedObject(self, kIndexKey, index,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (NSNumber*)frameCount {
...
- (void)setFrameCount:(NSNumber*)frameCount {
  objc_setAssociatedObject(self, kFrameCountKey, frameCount,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (NSNumber*)timestamp {
...
- (void)setTimestamp:(NSNumber*)timestamp {
  objc_setAssociatedObject(self, kTimestampKey, timestamp,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)setIndexDurations:(NSDictionary*)durations {
  objc_setAssociatedObject(self, kIndexDurationKey, durations,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)startGIFWithRunLoopMode:(NSString* const)runLoopMode {
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
    if (![[PlayGIFManager shared].gifViewHashTable containsObject:self] &&
        (self.gifData || self.gifPath)) {
      CGImageSourceRef gifSourceRef;
      if (self.gifData) {
        gifSourceRef = CGImageSourceCreateWithData(
            (__bridge CFDataRef)(self.gifData), NULL);
      } else {
        gifSourceRef = CGImageSourceCreateWithURL(
            (__bridge CFURLRef)[NSURL fileURLWithPath:self.gifPath], NULL);
      }
      if (!gifSourceRef) {
        return;
      }
      dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [[PlayGIFManager shared].gifSourceRefMapTable
            setObject:(__bridge id)(gifSourceRef)
               forKey:self];
        self.frameCount =
            [NSNumber numberWithInteger:CGImageSourceGetCount(gifSourceRef)];
        CGSize pxSize = [self GIFDimensionalSize];
        objc_setAssociatedObject(self, kPxSize,
                                 [NSValue valueWithCGSize:pxSize],
                                 OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        objc_setAssociatedObject(self, kGifLength,
                                 [self buildIndexAndReturnLength],
                                 OBJC_ASSOCIATION_RETAIN_NONATOMIC);
      });
    }
  });
  if (![PlayGIFManager shared].displayLink) {
    [PlayGIFManager shared].displayLink =
        [CADisplayLink displayLinkWithTarget:[PlayGIFManager shared]
                                    selector:@selector(play)];
    [[PlayGIFManager shared].displayLink addToRunLoop:[NSRunLoop mainRunLoop]
                                              forMode:runLoopMode];
  }
}
...
- (NSNumber*)buildIndexAndReturnLength {
  NSMutableDictionary* d = [[NSMutableDictionary alloc]
      initWithCapacity:[self.frameCount integerValue]];
  float l = 0;
...
- (void)play {
  self.timestamp =
      [NSNumber numberWithFloat:self.timestamp.floatValue +
                                [PlayGIFManager shared].displayLink.duration];

  float loopT =
      fmodf([self.timestamp floatValue], [[self gifLength] floatValue]);
  self.index = @([self indexForDuration:loopT]);
  CGImageSourceRef ref = (__bridge CGImageSourceRef)(
      [[PlayGIFManager shared].gifSourceRefMapTable objectForKey:self]);
  CGImageRef imageRef =
      CGImageSourceCreateImageAtIndex(ref, self.index.integerValue, NULL);
...
- (CGSize)GIFDimensionalSize {
  ...
  NSNumber* pixelWidth = (dict[(__bridge NSString*)kCGImagePropertyPixelWidth]);
  NSNumber* pixelHeight =
      (dict[(NSString*)CFBridgingRelease(kCGImagePropertyPixelHeight)]);

  CGSize sizeAsInProperties =
      CGSizeMake([pixelWidth floatValue], [pixelHeight floatValue]);
...
- (float)frameDurationAtIndex:(size_t)index {
  ...
  CFDictionaryRef dictRef =
      CGImageSourceCopyPropertiesAtIndex(ref, index, NULL);
  NSDictionary* dict = (__bridge NSDictionary*)dictRef;
  NSDictionary* gifDict =
      (dict[(__bridge NSString*)kCGImagePropertyGIFDictionary]);
  NSNumber* unclampedDelayTime =
      gifDict[(__bridge NSString*)kCGImagePropertyGIFUnclampedDelayTime];
  NSNumber* delayTime =
      gifDict[(__bridge NSString*)kCGImagePropertyGIFDelayTime];
  CFRelease(dictRef);
