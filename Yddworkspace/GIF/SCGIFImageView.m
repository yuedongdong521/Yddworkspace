...
- (BOOL)setGIFFile:(NSString*)gifFilePath {
  NSData* imageData = [NSData dataWithContentsOfFile:gifFilePath];
  return [self setGIFData:imageData];
}
...
- (void)loadImageData {
  ...
  for (int i = 0; i < [GIF_frames count]; i++) {
    UIImage* gitImg = [self getFrameAsImageAtIndex:i];
    if (gitImg) {
      [array addObject:gitImg];
    }
  }
  ...
  for (UIImage* image in array) {
    ...
    switch (frame.disposalMethod) {
      case 1:  // Do not dispose (draw over context)
        ...
        clipRect = CGRectMake(
            frame.area.origin.x,
            size.height - frame.area.size.height - frame.area.origin.y,
...
- (void)GIFReadDescriptor {
  ...
  NSMutableData* GIF_string =
      [NSMutableData dataWithData:[[NSString stringWithFormat:@"GIF89a"]
...
// Warning: The Copy Bundle Resources build phase contains this target's Info.plist file 'SCGifExample-Info.plist'.
...
- (bool)GIFGetBytes:(int)length {
  ...
  if ((NSInteger)[GIF_pointer length] >=
      dataPointer + length)  // Don't read across the edge of the file..
  {
    GIF_buffer = [[NSMutableData alloc]
        initWithData:[GIF_pointer
                         subdataWithRange:NSMakeRange(dataPointer, length)]];
    dataPointer += length;
    return YES;
  } else {
    return NO;
...
- (void)webImageManager:(SDWebImageManager *)imageManager ActivityIndicatorView:(UIActivityIndicatorView *)activityIndicatorView  FileName:(NSString*)fileName didFinishWithImage:(UIImage *)image drawRoundedRect:(BOOL)bDraw
{
    ...
    if (![self setGIFFile:fileName])
    {
        if (bDraw)
        {
            ...
            self.image = [UIImage createRoundedRectImage:image radius:r];
        }
        else
        {
            self.image = image;
