...
@property(atomic, weak, readwrite, nullable) id<GCDAsyncSocketDelegate>
    delegate;
...
- (void)getDelegate:
            (id<GCDAsyncSocketDelegate> __nullable* __nullable)delegatePtr
...
@property(atomic, assign, readwrite, getter=isIPv4PreferredOverIPv6)
...
- (void)unmarkSocketQueueTargetQueue:
    (dispatch_queue_t)socketQueuesPreviouslyConfiguredTargetQueue;
...
**/
+ (nullable NSMutableArray*)lookupHost:(NSString*)host
                                  port:(uint16_t)port
                                 error:(NSError**)errPtr;
...
newSocketQueueForConnectionFromAddress:(NSData*)address
