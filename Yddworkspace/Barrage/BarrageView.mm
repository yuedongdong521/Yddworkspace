...
//    }
     NSString *rankImageStr = @"rank_150";
    BarrageItemView *itemView = [[BarrageItemView alloc] initWithFrame:CGRectMake(ScreenWidth, kHeight(self) - (_maxLineCount == 1?lineHeight : lineSpac), itemW + lineHeight, lineHeight) WithNameWidth:nameW WithContentWidth:contentW];
    if (_isUniform) {
        itemView.durationTime = (ScreenWidth + kWidth(itemView)) / kSpeed;
    } else {
        itemView.durationTime = kDuration;
...
- (void)barrageStart:(BarrageItemView *)itemView
{
    ...
    for (int i = 0; i < lineValueCount; i++){
        ...
        if ([self judgeIsRunintoWithFirstBarrageItemView:itemView OldItemView:oldItemView]) {
            NSLog(@"judgeIsRunintoWithFirstBarrageItemView = YES i =%d", i);
            itemView.currentLineCount = i;
            [self playBarrayeView:itemView];
            break;
        } else if (i == lineValueCount - 1) {
            NSLog(@"judgeIsRunintoWithFirstBarrageItemView = NO i =%d", i);
...
- (void)playBarrayeView:(BarrageItemView *)itemView
{
    ...
    itemView.frame = CGRectMake(ScreenWidth, kHeight(self) - (itemView.currentLineCount + 1) * (_maxLineCount == 1?lineHeight : lineSpac),  kWidth(itemView), kHeight(itemView));
    ...
    [UIView animateWithDuration:itemView.durationTime delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
        ...
        itemView.frame = CGRectMake(kWidth(itemView) * (-1.0), kHeight(self) - (itemView.currentLineCount + 1) * (_maxLineCount == 1?lineHeight : lineSpac), kWidth(itemView), kHeight(itemView));
