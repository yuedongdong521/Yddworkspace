...
@interface AVCaptureSessionViewController () <
...
@property(nonatomic, strong) AVCaptureVideoDataOutput*
    videoDataOutput;  //原始视频针，用于获取实时图像以及视频录制

@property(nonatomic, strong) AVCaptureDeviceInput* audioInput;  //音频输入
@property(nonatomic, strong)
    AVCaptureAudioDataOutput* audioDataOutput;  //音频输出
//输出流
@property(nonatomic, strong)
    AVCaptureStillImageOutput* stillImageOutput;  //用于捕捉静态图片

@property(nonatomic, strong)
    AVCaptureMetadataOutput* metadataOutput;  //用于识别二维码以及人脸
//数据写入
...
@property(nonatomic, strong) ISH264Player* videoLayer;
...
- (void)initData {
  stillImageFlag = NO;  //照相模式
  videoDataFlag = YES;  //录制模式
  ...
  self.videoUrl = [[NSURL alloc]
      initFileURLWithPath:[filePath stringByAppendingPathComponent:@"ydd.mp4"]];
}
- (void)initAVCapture {
  //初始化视频输入设备（摄像头）
  [self initDevice];
...
- (void)initUI {
  ...
  [qrCodeBtn setImage:[UIImage imageNamed:@"scan_qrcode"]
             forState:UIControlStateNormal];
  [qrCodeBtn addTarget:self
                action:@selector(qrCodeBtnAction:)
      forControlEvents:UIControlEventTouchUpInside];
  ...
  [videoButton addTarget:self
        ...
        forControlEvents:UIControlEventTouchUpInside];
  [videoButton setTitleColor:[UIColor whiteColor]
                    forState:UIControlStateNormal];
  ...
  _qrCodeView = [[UIImageView alloc]
      initWithFrame:CGRectMake(ScreenWidth / 2.0 - 120,
                               ScreenHeight / 2.0 - 120, 240, 240)];
  ...
  _qrLineView = [[UIImageView alloc]
      initWithFrame:CGRectMake(0, 0, 240, lineImage.size.height)];
  ...
  _cancelButton.frame =
      CGRectMake(ScreenWidth / 4 - 25, ScreenHeight - 60, 50, 50);
  ...
  _saveButton.frame =
      CGRectMake(ScreenWidth / 4 * 3 - 25, ScreenHeight - 60, 50, 50);
  [_saveButton setTitle:@"保存" forState:UIControlStateNormal];
  [_saveButton setTitleColor:[UIColor whiteColor]
                    forState:UIControlStateNormal];
  [_saveButton setTitleShadowColor:[UIColor lightGrayColor]
                          forState:UIControlStateNormal];
  ...
  _foucImageView = [[UIImageView alloc]
      initWithImage:[UIImage imageNamed:@"camera_focus_red@2x"]];
...
- (void)qrCodeBtnAction:(UIButton*)btn {
  ...
  if (metadataOutputFlag) {
    stillImageFlag = NO;
    videoDataFlag = NO;
    [btn setImage:[UIImage imageNamed:@"scan_qrcode1"]
         forState:UIControlStateNormal];
    [self qrLineAnimate];
    ...
    stillImageFlag = YES;
    videoDataFlag = NO;
    [btn setImage:[UIImage imageNamed:@"scan_qrcode"]
         forState:UIControlStateNormal];
    _qrCodeView.hidden = YES;
  }
}
...
- (void)qrLineAnimate {
  ...
  [UIView animateWithDuration:3.0
                   ...
                   animations:^{
                     _qrLineView.frame =
                         CGRectMake(0, 240 - _qrLineView.frame.size.height, 240,
...
- (void)initDevice {
  ...
  if ([_device lockForConfiguration:nil]) {
    ...
    //自定义白平衡
    if ([_device isWhiteBalanceModeSupported:
                     AVCaptureWhiteBalanceModeContinuousAutoWhiteBalance]) {
      [_device setWhiteBalanceMode:
                   AVCaptureWhiteBalanceModeContinuousAutoWhiteBalance];
    }
    //自定义对焦
...
- (void)initMetadataOutput {
  ...
  //设置扫描区域
  _metadataOutput.rectOfInterest = _qrCodeView.frame;
...
- (UIButton*)torchButton {
  if (_torchButton == nil) {
    ...
    [_torchButton setImage:[UIImage imageNamed:@"flash_icon"]
                  forState:UIControlStateNormal];
    [_torchButton setImage:[UIImage imageNamed:@"flash_icon1"]
                  forState:UIControlStateSelected];
    [_torchButton addTarget:self
                     action:@selector(openTorch:)
           forControlEvents:UIControlEventTouchUpInside];
  }
  return _torchButton;
}
...
- (UIButton*)takePhotoButton {
  if (_takePhotoButton == nil) {
    ...
    _takePhotoButton.frame =
        CGRectMake(ScreenWidth / 2.0 - 35, ScreenHeight - 80, 70, 70);
    ...
    [_takePhotoButton addTarget:self
               ...
               forControlEvents:UIControlEventTouchUpInside];
    [_takePhotoButton setTitleColor:[UIColor whiteColor]
                           forState:UIControlStateNormal];
  }
  ...
  dispatch_async(self.writeQueue, ^{
    @autoreleasepool {
      ...
      if (!self.canWrite && mediaType == AVMediaTypeVideo) {
        ...
        [self.assetWriter
            startSessionAtSourceTime:CMSampleBufferGetPresentationTimeStamp(
                                         sampleBuffer)];
        self.canWrite = YES;
      }
...
- (void)startRecordVideo {
  ...
  if ([[NSFileManager defaultManager] fileExistsAtPath:self.videoUrl.path]) {
    [[NSFileManager defaultManager] removeItemAtPath:self.videoUrl.path
                                               error:nil];
  }
  ...
  _assetWriterVideoInput = [AVAssetWriterInput
      assetWriterInputWithMediaType:AVMediaTypeVideo
                     outputSettings:self.videoCompressionSettings];
  ...
  _assetWriterAudioInput = [AVAssetWriterInput
      assetWriterInputWithMediaType:AVMediaTypeAudio
                     outputSettings:self.audioCompressionSettings];
    ...
    dispatch_async(self.writeQueue, ^{
      [_assetWriter finishWritingWithCompletionHandler:^{
        if ([self judgelibrary]) {
          ...
          [lib writeVideoAtPathToSavedPhotosAlbum:weakself.videoUrl
...
- (void)saveImageToPhonoAlbum:(UIImage*)savedimage {
  UIImageWriteToSavedPhotosAlbum(
      savedimage, self, @selector(image:didFinishSavingWithError:contextInfo:),
      NULL);
}
...
- (CVPixelBufferRef)addWaiterImage:(CMSampleBufferRef)sampleBuffer {
  ...
  //  status = CMSampleBufferCreateForImageBuffer(kCFAllocatorDefault, pixelBuffer, true, NULL, NULL, videoInfo, &sampleTime, &newSampleBuffer);
...
#else
    AVMetadataObject* faceData = [self.previewLayer
...
- (void)metadataFaceForArray:(NSArray*)array {
  ...
  if (metadataObject.type == AVMetadataObjectTypeFace) {
    AVMetadataObject* objec = [self.previewLayer
        transformedMetadataObjectForMetadataObject:metadataObject];
    NSLog(@"************face: %@", objec);
