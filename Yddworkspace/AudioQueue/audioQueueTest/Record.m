...
static void AQInputCallback(void* inUserData, AudioQueueRef inAudioQueue,
...
- (id)init {
  ...
  if (self) {
    ...
    AudioQueueNewInput(&aqc.mDataFormat, AQInputCallback,
                       (__bridge void*)(self), NULL, kCFRunLoopCommonModes, 0,
                       &aqc.queue);
...
- (void)initFileHandel {
  NSString* audioPath = NSSearchPathForDirectoriesInDomains(
                           NSDocumentDirectory, NSUserDomainMask, YES)
                           .lastObject;
  audioPath = [audioPath stringByAppendingPathComponent:@"test.aac"];
...
//   [_fileHandle writeData:data];
  ...
  if (_processAudioBuffer) {
    _processAudioBuffer(buffer->mAudioData, buffer->mAudioDataByteSize);
  }
