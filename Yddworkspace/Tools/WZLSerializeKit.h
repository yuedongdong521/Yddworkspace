...
#define WZLSERIALIZE_CODER_DECODER()                                                                \
  ...
  -(id)initWithCoder : (NSCoder*)coder {                                                            \
    ...
    while (cls != [NSObject class]) {                                                               \
      ...
      Ivar* ivarList = bIsSelfClass                                                                 \
                           ? class_copyIvarList([cls class], &iVarCount)                            \
                           : NULL; /*变量列表，含属性以及私有变量*/                                 \
      objc_property_t* propList =                                                                   \
          bIsSelfClass                                                                              \
              ? NULL                                                                                \
              : class_copyPropertyList(cls, &propVarCount); /*属性列表*/                            \
      sharedVarCount = bIsSelfClass ? iVarCount : propVarCount;                                     \
                                                                                                    \
      for (int i = 0; i < sharedVarCount; i++) {                                                    \
        const char* varName = bIsSelfClass                                                          \
                                  ? ivar_getName(*(ivarList + i))                                   \
                                  : property_getName(*(propList + i));                              \
        NSString* key = [NSString stringWithUTF8String:varName];                                    \
        id varValue = [coder decodeObjectForKey:key];                                               \
        NSArray* filters =                                                                          \
            @[ @"superclass", @"description", @"debugDescription", @"hash" ];                       \
...
#define WZLSERIALIZE_UNARCHIVE(__objToStoreData__, __key__, __filePath__)      \
  ...
  NSKeyedUnarchiver* unarchiver =                                              \
      [[NSKeyedUnarchiver alloc] initForReadingWithData:dedata];               \
