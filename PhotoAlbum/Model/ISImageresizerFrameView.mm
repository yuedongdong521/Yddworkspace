...
@interface ISImageresizerFrameView ()
...
@implementation ISImageresizerFrameView {
...
- (void)setFillColor:(UIColor*)fillColor {
  ...
  _fillColor = [UIColor colorWithRed:_fillRgba.jp_r
                               green:_fillRgba.jp_g
                                blue:_fillRgba.jp_b
                               alpha:_fillRgba.jp_a * _maskAlpha];
  _clearColor = [UIColor colorWithRed:_fillRgba.jp_r
                                green:_fillRgba.jp_g
                                 blue:_fillRgba.jp_b
                                alpha:0];
...
- (void)setIsRotatedAutoScale:(BOOL)isRotatedAutoScale {
  ...
  if (self.superview)
    [self updateMaxResizeFrameWithDirection:_rotationDirection];
}

- (void)setIsCanRecovery:(BOOL)isCanRecovery {
  _isCanRecovery = isCanRecovery;
  if (self.imageresizerIsCanRecovery) {
...
- (void)setIsPrepareToScale:(BOOL)isPrepareToScale {
  ...
  if (self.imageresizerIsPrepareToScale) {
...
- (instancetype)initWithFrame:(CGRect)frame
       ...
       imageresizerIsCanRecovery:
           (JPImageresizerIsCanRecoveryBlock)imageresizerIsCanRecovery
    imageresizerIsPrepareToScale:
        (JPImageresizerIsPrepareToScaleBlock)imageresizerIsPrepareToScale {
...
- (UIBezierPath*)dotPathWithPosition:(CGPoint)position {
  ...
  UIBezierPath* dotPath = [UIBezierPath
      bezierPathWithOvalInRect:CGRectMake(position.x - dotWH * 0.5,
                                          position.y - dotWH * 0.5, dotWH,
                                          dotWH)];
  return dotPath;
}
...
- (UIBezierPath*)linePathWithLinePosition:(LinePosition)linePosition
...
- (BOOL)imageresizerFrameIsEqualImageViewFrame {
  ...
  CGFloat resizeWHScale =
      (self.rotationDirection == JPImageresizerVerticalUpDirection ||
       self.rotationDirection == JPImageresizerVerticalDownDirection)
          ? _resizeWHScale
          : (1.0 / _resizeWHScale);
...
- (void)hideOrShowBlurEffect:(BOOL)isHide
  ...
  if (duration > 0 && ((_blurContentView != nil) ||
                       (_blurContentView == nil &&
                        ![self imageresizerFrameIsFullImageViewFrame]))) {
    CGFloat fromOpacity = isHide ? 1 : 0;
    CABasicAnimation* anim = [CABasicAnimation
        animationWithKeyPath:aKeyPath(self.blurEffectView.layer, opacity)];
    anim.fillMode = kCAFillModeBackwards;
    anim.fromValue = @(fromOpacity);
    anim.toValue = @(toOpacity);
    anim.duration = duration;
    anim.timingFunction =
        [CAMediaTimingFunction functionWithName:_kCAMediaTimingFunction];
...
- (void)hideOrShowFrameLine:(BOOL)isHide
            animateDuration:(NSTimeInterval)duration {
...
- (void)updateImageresizerFrame:(CGRect)imageresizerFrame
  ...
  if (_frameType == JPConciseFrameType ||
      _frameType == JPConciseWithoutOtherDotFrameType) {
    leftTopDotPath =
        [self dotPathWithPosition:CGPointMake(imageresizerX, imageresizerY)];
    leftBottomDotPath =
        [self dotPathWithPosition:CGPointMake(imageresizerX, imageresizerMaxY)];
    rightTopDotPath =
        [self dotPathWithPosition:CGPointMake(imageresizerMaxX, imageresizerY)];
    rightBottomDotPath = [self
        dotPathWithPosition:CGPointMake(imageresizerMaxX, imageresizerMaxY)];

    if (_frameType == JPConciseFrameType) {
      leftMidDotPath = [self
          dotPathWithPosition:CGPointMake(imageresizerX, imageresizerMidY)];
      rightMidDotPath = [self
          dotPathWithPosition:CGPointMake(imageresizerMaxX, imageresizerMidY)];
      topMidDotPath = [self
          dotPathWithPosition:CGPointMake(imageresizerMidX, imageresizerY)];
      bottomMidDotPath = [self
          dotPathWithPosition:CGPointMake(imageresizerMidX, imageresizerMaxY)];
    }

  } else {
    leftTopDotPath =
        [self arrPathWithPosition:CGPointMake(imageresizerX, imageresizerY)
                         rectHorn:LeftTop];
    leftBottomDotPath =
        [self arrPathWithPosition:CGPointMake(imageresizerX, imageresizerMaxY)
                         rectHorn:LeftBottom];
    rightTopDotPath =
        [self arrPathWithPosition:CGPointMake(imageresizerMaxX, imageresizerY)
                         rectHorn:RightTop];
    rightBottomDotPath = [self
        arrPathWithPosition:CGPointMake(imageresizerMaxX, imageresizerMaxY)
                   rectHorn:RightBottom];
    ...
    horTopLinePath =
        [self linePathWithLinePosition:HorizontalTop
                              location:CGPointMake(imageresizerX,
                                                   imageresizerY + oneThirdH)
                                length:imageresizerW];
    horBottomLinePath = [self
        linePathWithLinePosition:HorizontalBottom
                        location:CGPointMake(imageresizerX,
                                             imageresizerY + oneThirdH * 2)
                          length:imageresizerW];
    verLeftLinePath =
        [self linePathWithLinePosition:VerticalLeft
                              location:CGPointMake(imageresizerX + oneThirdW,
                                                   imageresizerY)
                                length:imageresizerH];
    verRightLinePath = [self
        linePathWithLinePosition:VerticalRight
                        location:CGPointMake(imageresizerX + oneThirdW * 2,
                                             imageresizerY)
                          length:imageresizerH];
  }
    ...
    void (^layerPathAnimate)(CAShapeLayer* layer, UIBezierPath* path) =
        ^(CAShapeLayer* layer, UIBezierPath* path) {
          CABasicAnimation* anim =
              [CABasicAnimation animationWithKeyPath:aKeyPath(layer, path)];
          anim.fillMode = kCAFillModeBackwards;
...
- (void)updateImageOriginFrameWithDirection:
    (JPImageresizerRotationDirection)rotationDirection {
  [self removeTimer];
...
- (BOOL)updateRotationDirection:
  ...
  if (!_isArbitrarily) {
    ...
    BOOL isHor2Ver =
        ((_rotationDirection == JPImageresizerHorizontalLeftDirection ||
...
- (void)updateMaxResizeFrameWithDirection:
  ...
  if (direction == JPImageresizerVerticalUpDirection ||
    ...
    if (self.isRotatedAutoScale) {
      ...
      _sizeScale = _verSizeScale;
      x = (self.bounds.size.width - _contentSize.height) * 0.5 +
          _verBaseMargin / _sizeScale;
      y = (self.bounds.size.height - _contentSize.width) * 0.5 +
          _horBaseMargin / _sizeScale;
      w = self.bounds.size.width - 2 * x;
      h = self.bounds.size.height - 2 * y;
    }
  }
...
- (void)updateImageresizerFrameWithAnimateDuration:(NSTimeInterval)duration
                                    isAdjustResize:(BOOL)adjustResize {
  ...
  UIEdgeInsets contentInset =
      [self scrollViewContentInsetWithAdjustResizeFrame:adjustResizeFrame];
  ...
  contentOffset.x =
      -contentInset.left + convertPoint.x * self.scrollView.zoomScale;
  contentOffset.y =
      -contentInset.top + convertPoint.y * self.scrollView.zoomScale;
  ...
  void (^completeBlock)(void) = ^{
    ...
    sSelf.scrollView.minimumZoomScale =
        [sSelf scrollViewMinZoomScaleWithResizeSize:adjustResizeFrame.size];
...
- (CGRect)adjustResizeFrame {
  CGFloat resizerWHScale =
      _isArbitrarily ? (self.imageresizeW / self.imageresizeH) : _resizeWHScale;
  ...
  CGFloat adjustResizeX =
      self.maxResizeX + (self.maxResizeW - adjustResizeW) * 0.5;
  CGFloat adjustResizeY =
      self.maxResizeY + (self.maxResizeH - adjustResizeH) * 0.5;
  return CGRectMake(adjustResizeX, adjustResizeY, adjustResizeW, adjustResizeH);
}

- (UIEdgeInsets)scrollViewContentInsetWithAdjustResizeFrame:
    (CGRect)adjustResizeFrame {
  // scrollView宽高跟self一样，上下左右不需要额外添加Space
  CGFloat top = adjustResizeFrame.origin.y;  // + veSpace?
  CGFloat bottom =
      self.bounds.size.height - CGRectGetMaxY(adjustResizeFrame);  // + veSpace?
  CGFloat left = adjustResizeFrame.origin.x;  // + hoSpace?
  CGFloat right =
      self.bounds.size.width - CGRectGetMaxX(adjustResizeFrame);  // + hoSpace?
...
- (void)checkIsCanRecovery {
  BOOL isVerticalityMirror =
      self.isVerticalityMirror ? self.isVerticalityMirror() : NO;
  BOOL isHorizontalMirror =
      self.isHorizontalMirror ? self.isHorizontalMirror() : NO;
  if (isVerticalityMirror || isHorizontalMirror) {
    self.isCanRecovery = YES;
    return;
  }

  CGPoint convertCenter =
      [self convertPoint:CGPointMake(CGRectGetMidX(self.bounds),
                                     CGRectGetMidY(self.bounds))
                  toView:self.imageView];
  CGPoint imageViewCenter = CGPointMake(CGRectGetMidX(self.imageView.bounds),
                                        CGRectGetMidY(self.imageView.bounds));
  BOOL isSameCenter =
      (labs((NSInteger)convertCenter.x - (NSInteger)imageViewCenter.x) <= 1 &&
...
- (UIImage*)getTargetDirectionImage:(UIImage*)image
                  verticalityMirror:(BOOL)verticalityMirror
...
- (void)updateImageresizerFrameWithVerBaseMargin:(CGFloat)verBaseMargin
                                   horBaseMargin:(CGFloat)horBaseMargin {
  _verBaseMargin = verBaseMargin;
  _horBaseMargin = horBaseMargin;
...
- (void)rotationWithDirection:(JPImageresizerRotationDirection)direction
             rotationDuration:(NSTimeInterval)rotationDuration {
  [self removeTimer];
...
- (void)verticalityMirrorWithDiffX:(CGFloat)diffX {
  CGFloat w = !self.isHorizontalDirection ? self.bounds.size.width
...
- (void)horizontalMirrorWithDiffY:(CGFloat)diffY {
  CGFloat h = !self.isHorizontalDirection ? self.bounds.size.height
...
- (void)recovery {
  ...
  CGRect adjustResizeFrame =
      _isArbitrarily ? [self baseImageresizerFrame] : [self adjustResizeFrame];

  UIEdgeInsets contentInset =
      [self scrollViewContentInsetWithAdjustResizeFrame:adjustResizeFrame];

  CGFloat minZoomScale =
      [self scrollViewMinZoomScaleWithResizeSize:adjustResizeFrame.size];

  CGFloat contentOffsetX =
      -contentInset.left +
      (_baseImageW * minZoomScale - adjustResizeFrame.size.width) * 0.5;
  CGFloat contentOffsetY =
      -contentInset.top +
      (_baseImageH * minZoomScale - adjustResizeFrame.size.height) * 0.5;
...
- (void)imageresizerWithComplete:(void (^)(UIImage*))complete {
  ...
  CGRect cropFrame =
      (self.isCanRecovery || self.resizeWHScale > 0)
          ? [self convertRect:self.imageresizerFrame toView:self.imageView]
  ...
  dispatch_async(dispatch_get_global_queue(0, 0), ^{
    ...
    if (isVerticalityMirror)
      cropRect.origin.x = image.size.width - CGRectGetMaxX(cropRect);
    if (isHorizontalMirror)
      cropRect.origin.y = image.size.height - CGRectGetMaxY(cropRect);

    CGImageRef imgRef = CGImageCreateWithImageInRect(image.CGImage, cropRect);

    // 有小数的情况下，边界会多出白线，需要把小数点去掉
    CGSize cropSize =
        CGSizeMake(floor(cropFrame.size.width), floor(cropFrame.size.height));
    ...
    CGContextDrawImage(
        context, CGRectMake(0, 0, cropSize.width, cropSize.height), imgRef);
...
- (void)panHandle:(UIPanGestureRecognizer*)panGR {
  ...
  if (panGR.state == UIGestureRecognizerStateChanged)
    [self panChangedHandleWithTranslation:translation];
...
- (void)panBeganHandleWithLocation:(CGPoint)location {
  ...
  CGRect leftTopRect =
      CGRectMake(x - halfScopeWH, y - halfScopeWH, scopeWH, scopeWH);
  CGRect leftBotRect =
      CGRectMake(x - halfScopeWH, maxY - halfScopeWH, scopeWH, scopeWH);
  CGRect rightTopRect =
      CGRectMake(maxX - halfScopeWH, y - halfScopeWH, scopeWH, scopeWH);
  CGRect rightBotRect =
      CGRectMake(maxX - halfScopeWH, maxY - halfScopeWH, scopeWH, scopeWH);
  CGRect leftMidRect =
      CGRectMake(x - halfScopeWH, midY - halfScopeWH, scopeWH, scopeWH);
  CGRect rightMidRect =
      CGRectMake(maxX - halfScopeWH, midY - halfScopeWH, scopeWH, scopeWH);
  CGRect topMidRect =
      CGRectMake(midX - halfScopeWH, y - halfScopeWH, scopeWH, scopeWH);
  CGRect botMidRect =
      CGRectMake(midX - halfScopeWH, maxY - halfScopeWH, scopeWH, scopeWH);
