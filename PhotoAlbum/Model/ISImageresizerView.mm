...
@interface ISImageresizerView () <UIScrollViewDelegate>
@property(nonatomic, weak) UIScrollView* scrollView;
@property(nonatomic, weak) UIImageView* imageView;
@property(nonatomic, weak) ISImageresizerFrameView* frameView;

@property(nonatomic, strong) NSMutableArray* allDirections;
@property(nonatomic, assign) NSInteger directionIndex;
@end

@implementation ISImageresizerView {
...
- (void)setupBase {
  ...
  self.allDirections = [@[
    @(JPImageresizerVerticalUpDirection),
...
- (void)setupFrameViewWithMaskType:(JPImageresizerMaskType)maskType
                    ...
                    animationCurve:(JPAnimationCurve)animationCurve
                     ...
                     resizeWHScale:(CGFloat)resizeWHScale
                isCanRecoveryBlock:
                    (JPImageresizerIsCanRecoveryBlock)isCanRecoveryBlock
             isPrepareToScaleBlock:
                 (JPImageresizerIsPrepareToScaleBlock)isPrepareToScaleBlock {
  ISImageresizerFrameView* frameView =
      [[ISImageresizerFrameView alloc] initWithFrame:self.scrollView.frame
...
- (void)updateSubviewLayouts {
  ...
  [self.frameView updateImageresizerFrameWithVerBaseMargin:_verBaseMargin
                                             horBaseMargin:_horBaseMargin];
}
...
- (void)setMirror:(BOOL)isHorizontalMirror animated:(BOOL)isAnimated {
  ...
  void (^animateBlock)(CATransform3D aTransform) = ^(CATransform3D aTransform) {
    ...
    if (isHorizontalMirror) {
      [self.frameView horizontalMirrorWithDiffY:(mirror ? _contentInsets.bottom
                                                        : _contentInsets.top)];
    } else {
      [self.frameView
          verticalityMirrorWithDiffX:(mirror ? _contentInsets.right
                                             : _contentInsets.left)];
    }
  };
...
- (void)rotation {
  ...
  [UIView animateWithDuration:duration
                   ...
                   animations:^{
                     ...
                     [self.frameView rotationWithDirection:direction
...
blurMaskTypeConfigureWithResizeImage:(UIImage*)resizeImage
                             isLight:(BOOL)isLight
                                make:(void (^)(JPImageresizerConfigure*))make {
  JPImageresizerMaskType maskType =
      isLight ? JPLightBlurMaskType : JPDarkBlurMaskType;
  JPImageresizerConfigure* configure = [self
      defaultConfigureWithResizeImage:resizeImage
                                 make:^(JPImageresizerConfigure* configure) {
                                   configure.jp_maskType(maskType).jp_maskAlpha(
                                       0.3);
                                 }];
  if (make) {
    make(configure);
...
- (JPImageresizerConfigure* (^)(JPImageresizerFrameType frameType))
    jp_frameType {
...
- (JPImageresizerConfigure* (^)(JPAnimationCurve animationCurve))
    jp_animationCurve {
...
- (JPImageresizerConfigure* (^)(BOOL isClockwiseRotation))
    jp_isClockwiseRotation {
