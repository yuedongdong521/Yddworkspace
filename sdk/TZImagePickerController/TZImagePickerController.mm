...
- (void)viewDidLoad {
  ...
  self.oKButtonTitleColorNormal = [UIColor colorWithRed:0 green:0 blue:31 / 255.0 alpha:1];
  self.oKButtonTitleColorDisabled = [UIColor colorWithRed:253 / 255.0 green:253 / 255.0 blue:253 / 255.0 alpha:1];
  if (iOS7Later) {
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount
                              delegate:(id<TZImagePickerControllerDelegate>)
                                           delegate {
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount
  ...
  TZAlbumPickerController* albumPickerVc =
      [[TZAlbumPickerController alloc] init];
  albumPickerVc.columnNumber = columnNumber;
  self = [super initWithRootViewController:albumPickerVc];
  if (self) {
    self.maxImagesCount = maxImagesCount > 0
                              ? maxImagesCount
                              : 9;  // Default is 9 / 默认最大可选9张图片
    ...
    if (![[TZImageManager manager] authorizationStatusAuthorized]) {
      ...
      [_settingBtn setTitle:self.settingBtnTitleStr
...
- (instancetype)initWithSelectedAssets:(NSMutableArray*)selectedAssets
  ...
  if (self) {
    ...
    [previewVc setDoneButtonClickBlockWithPreviewType:^(
          ...
          dismissViewControllerAnimated:YES
                             completion:^{
                               if (weakSelf.didFinishPickingPhotosHandle) {
                                 weakSelf.didFinishPickingPhotosHandle(
                                     photos, assets, isSelectOriginalPhoto);
                               }
                             }];
    }];
  }
...
- (instancetype)initCropTypeWithAsset:(id)asset
                                photo:(UIImage*)photo
                           completion:(void (^)(UIImage* cropImage,
                                                id asset))completion {
  TZPhotoPreviewController* previewVc = [[TZPhotoPreviewController alloc] init];
  self = [super initWithRootViewController:previewVc];
  if (self) {
...
- (void)pushPhotoPickerVc {
  ...
  if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
    TZPhotoPickerController* photoPickerVc =
        [[TZPhotoPickerController alloc] init];
    photoPickerVc.isFirstAppear = YES;
    photoPickerVc.columnNumber = self.columnNumber;
    [[TZImageManager manager]
        getCameraRollAlbum:self.allowPickingVideo
  ...
  if (iOS8Later) {
    ...
    [alertController
        addAction:[UIAlertAction
                      actionWithTitle:[NSBundle tz_localizedStringForKey:@"OK"]
...
- (void)showProgressHUD {
  if (!_progressHUD) {
    ...
    _HUDContainer.frame = CGRectMake((self.view.tz_width - 120) / 2,
                                     (self.view.tz_height - 90) / 2, 120, 90);
    _HUDContainer.layer.cornerRadius = 8;
    _HUDContainer.clipsToBounds = YES;
    _HUDContainer.backgroundColor = [UIColor darkGrayColor];
    _HUDContainer.alpha = 0.7;

    _HUDIndicatorView = [[UIActivityIndicatorView alloc]
  ...
  dispatch_after(
      dispatch_time(DISPATCH_TIME_NOW, (int64_t)(self.timeout * NSEC_PER_SEC)),
...
- (void)setCircleCropRadius:(NSInteger)circleCropRadius {
  ...
  _cropRect = CGRectMake(self.view.tz_width / 2 - circleCropRadius,
                         self.view.tz_height / 2 - _circleCropRadius,
                         _circleCropRadius * 2, _circleCropRadius * 2);
}
...
- (CGRect)cropRect {
  ...
  return CGRectMake(0, (self.view.tz_height - self.view.tz_width) / 2,
                    cropViewWH, cropViewWH);
}
...
- (void)setSelectedAssets:(NSMutableArray*)selectedAssets {
  ...
  for (id asset in selectedAssets) {
    TZAssetModel* model =
        [TZAssetModel modelWithAsset:asset type:TZAssetModelMediaTypePhoto];
...
- (void)setAllowPickingImage:(BOOL)allowPickingImage {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:allowPickingImageStr
                                            forKey:@"tz_allowPickingImage"];
...
- (void)setAllowPickingVideo:(BOOL)allowPickingVideo {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:allowPickingVideoStr
                                            forKey:@"tz_allowPickingVideo"];
  [[NSUserDefaults standardUserDefaults] synchronize];
}

- (void)setSortAscendingByModificationDate:
    (BOOL)sortAscendingByModificationDate {
  _sortAscendingByModificationDate = sortAscendingByModificationDate;
  [TZImageManager manager].sortAscendingByModificationDate =
    ...
    if ([[UIApplication sharedApplication] canOpenURL:privacyUrl]) {
      ...
      NSString* message =
          [NSBundle tz_localizedStringForKey:
                        @"Can not jump to the privacy settings page, please go "
                        @"to the settings page by self, thank you"];
...
- (void)callDelegateMethod {
  ...
  if ([self.pickerDelegate
          respondsToSelector:@selector(tz_imagePickerControllerDidCancel:)]) {
...
@interface TZAlbumPickerController () <UITableViewDataSource,
                                       UITableViewDelegate> {
...
- (void)viewDidLoad {
  ...
  self.navigationItem.rightBarButtonItem =
      [[UIBarButtonItem alloc] initWithTitle:imagePickerVc.cancelBtnTitleStr
  ...
  self.navigationItem.backBarButtonItem = [[UIBarButtonItem alloc]
      initWithTitle:[NSBundle tz_localizedStringForKey:@"Back"]
              style:UIBarButtonItemStylePlain
             target:nil
             action:nil];
}
...
- (void)configTableView {
  TZImagePickerController* imagePickerVc =
      (TZImagePickerController*)self.navigationController;
  [[TZImageManager manager]
           getAllAlbums:imagePickerVc.allowPickingVideo
      allowPickingImage:imagePickerVc.allowPickingImage
             completion:^(NSArray<TZAlbumModel*>* models) {
               ...
               if (!_tableView) {
                 ...
                 [_tableView registerClass:[TZAlbumCell class]
                     forCellReuseIdentifier:@"TZAlbumCell"];
...
- (UITableViewCell*)tableView:(UITableView*)tableView
  ...
  TZAlbumCell* cell =
      [tableView dequeueReusableCellWithIdentifier:@"TZAlbumCell"
                                      forIndexPath:indexPath];
  //    TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
  //    cell.selectedCountButton.backgroundColor = imagePickerVc.oKButtonTitleColorNormal;
  cell.selectedCountButton.backgroundColor = [UIColor colorWithRed:246 / 255.0 green:212 / 255.0 blue:48 / 255.0 alpha:1];
...
+ (UIImage*)imageNamedFromMyBundle:(NSString*)name {
  UIImage* image =
      [UIImage imageNamed:[@"TZImagePickerController.bundle"
                              stringByAppendingPathComponent:name]];
  if (image) {
    return image;
  } else {
    image =
        [UIImage imageNamed:[@"Frameworks/TZImagePickerController.framework/"
