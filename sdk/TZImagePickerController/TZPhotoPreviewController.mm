...
@interface TZPhotoPreviewController () <UICollectionViewDataSource,
...
- (void)viewDidLoad {
  ...
  if (!self.models.count) {
    self.models =
        [NSMutableArray arrayWithArray:_tzImagePickerVc.selectedModels];
    _assetsTemp =
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  if (_currentIndex)
    [_collectionView
        setContentOffset:CGPointMake((self.view.tz_width + 20) * _currentIndex,
                                     0)
                animated:NO];
...
- (void)configCustomNaviBar {
  ...
  _naviBar.backgroundColor = [UIColor colorWithRed:(34 / 255.0)
                                             green:(34 / 255.0)
                                              blue:(34 / 255.0)
                                             alpha:0.7];
  CGFloat backButtonOrigionY = 10.f;
  if (IS_IPHONE_X) {
    backButtonOrigionY = kStatusBarHeight - 1;
  }
  _backButton = [[UIButton alloc]
      initWithFrame:CGRectMake(10, backButtonOrigionY, 44, 44)];
  [_backButton setImage:[UIImage imageNamedFromMyBundle:@"navi_back.png"]
  ...
  // 选中按钮
  CGFloat selectButtonOrigionY = 15.f;
  if (IS_IPHONE_X) {
    selectButtonOrigionY = kStatusBarHeight - 1;
  }
  _selectButton =
      [[UIButton alloc] initWithFrame:CGRectMake(self.view.tz_width - 54,
                                                 selectButtonOrigionY, 45, 45)];
  [_selectButton setImage:[UIImage imageNamedFromMyBundle:@"photo_original_def@2x"]
                 forState:UIControlStateNormal];  //
  [_selectButton setImage:[UIImage imageNamedFromMyBundle:@"photo_original_sel@2x"]
...
- (void)configBottomToolBar {
  ...
  if (IS_IPHONE_X) {
    toolBarHeight = 44.f + IS_TABBAR_ADD_HEIGHT;
  }
  _toolBar = [[UIView alloc]
      initWithFrame:CGRectMake(0, self.view.tz_height - toolBarHeight,
                               self.view.tz_width, toolBarHeight)];
  static CGFloat rgb = 34 / 255.0;
  _toolBar.backgroundColor =
      [UIColor colorWithRed:rgb green:rgb blue:rgb alpha:0.7];

  TZImagePickerController* _tzImagePickerVc =
      (TZImagePickerController*)self.navigationController;
  if (_tzImagePickerVc.allowPickingOriginalPhoto) {
    CGFloat fullImageWidth =
        [_tzImagePickerVc.fullImageBtnTitleStr
    ...
    [_originalPhotoButton setTitle:_tzImagePickerVc.fullImageBtnTitleStr
                          forState:UIControlStateNormal];
    [_originalPhotoButton setTitle:_tzImagePickerVc.fullImageBtnTitleStr
                          forState:UIControlStateSelected];
    [_originalPhotoButton setTitleColor:[UIColor lightGrayColor]
                               forState:UIControlStateNormal];
    [_originalPhotoButton setTitleColor:[UIColor whiteColor]
                               forState:UIControlStateSelected];
    [_originalPhotoButton
        setImage:[UIImage
                     imageNamedFromMyBundle:_tzImagePickerVc
  ...
  if (_tzImagePickerVc.selectedModels.count > 0) {
    ...
    buttonTitle =
        [NSString stringWithFormat:@"确定(%zd/%zd)",
                                   _tzImagePickerVc.selectedModels.count,
                                   _tzImagePickerVc.maxImagesCount];
  }
  _doneButton.frame =
...
- (void)configCollectionView {
  ...
  _collectionView = [[UICollectionView alloc]
             initWithFrame:CGRectMake(-10, 0, self.view.tz_width + 20,
                                      self.view.tz_height)
  ...
  _collectionView.contentSize =
      CGSizeMake(self.models.count * (self.view.tz_width + 20), 0);
  [self.view addSubview:_collectionView];
  [_collectionView registerClass:[TZPhotoPreviewCell class]
      forCellWithReuseIdentifier:@"TZPhotoPreviewCell"];
}
...
- (void)configCropView {
  ...
  if (!_tzImagePickerVc.showSelectBtn && _tzImagePickerVc.allowCrop) {
    ...
    [TZImageCropManager
        overlayClippingWithView:_cropBgView
                       cropRect:_tzImagePickerVc.cropRect
                  containerView:self.view
                 needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)select:(UIButton*)selectButton {
  ...
  if (!selectButton.isSelected) {
    ...
    if (_tzImagePickerVc.selectedModels.count >=
        _tzImagePickerVc.maxImagesCount) {
        ...
        [_tzImagePickerVc
            showAlertWithTitle:[NSBundle
                                   tz_localizedStringForKey:
                                       @"Select the video when in multi state, "
                                       @"we will handle the video as a photo"]];
      }
    }
  } else {
    NSArray* selectedModels =
        [NSArray arrayWithArray:_tzImagePickerVc.selectedModels];
  ...
  [self refreshNaviBarAndBottomBarState];
  if (model.isSelected) {
    [UIView showOscillatoryAnimationWithLayer:selectButton.imageView.layer
...
- (void)doneButtonClick {
  ...
  if (_progress > 0 && _progress < 1) {
    [_tzImagePickerVc
        showAlertWithTitle:[NSBundle tz_localizedStringForKey:
  ...
  if (_tzImagePickerVc.selectedModels.count == 0 &&
      _tzImagePickerVc.minImagesCount <= 0) {
    TZAssetModel* model = _models[_currentIndex];
    [_tzImagePickerVc.selectedModels addObject:model];
  }
  if (_tzImagePickerVc.allowCrop) {  // 裁剪状态
    NSIndexPath* indexPath =
        [NSIndexPath indexPathForItem:_currentIndex inSection:0];
    TZPhotoPreviewCell* cell =
        (TZPhotoPreviewCell*)[_collectionView cellForItemAtIndexPath:indexPath];
    UIImage* cropedImage =
        [TZImageCropManager cropImageView:cell.previewView.imageView
                                   toRect:_tzImagePickerVc.cropRect
                                zoomScale:cell.previewView.scrollView.zoomScale
                            containerView:self.view];
    if (_tzImagePickerVc.needCircleCrop) {
  ...
  if (self.doneButtonClickBlockWithPreviewType) {
    self.doneButtonClickBlockWithPreviewType(self.photos,
                                             _tzImagePickerVc.selectedAssets,
                                             self.isSelectOriginalPhoto);
  }
}
...
- (void)originalPhotoButtonClick {
  ...
  if (_isSelectOriginalPhoto) {
    ...
    if (!_selectButton.isSelected) {
      ...
      if (_tzImagePickerVc.selectedModels.count <
              _tzImagePickerVc.maxImagesCount &&
          _tzImagePickerVc.showSelectBtn) {
...
- (UICollectionViewCell*)collectionView:(UICollectionView*)collectionView
  ...
  TZPhotoPreviewCell* cell = [collectionView
      dequeueReusableCellWithReuseIdentifier:@"TZPhotoPreviewCell"
...
- (void)showPhotoBytes {
  [[TZImageManager manager]
      getPhotosBytesWithArray:@[ _models[_currentIndex] ]
