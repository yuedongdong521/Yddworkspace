...
- (id)init {
  if (self = [super initWithFragmentShaderFromString:
                        kGPUImageBeautifyFragmentShaderString]) {
...
- (void)setIntensity:(CGFloat)intensity {
  ...
  [self setFloat:intensity
      forUniform:smoothDegreeUniform
...
- (id)init;
{
  ...
  // [hsbFilter adjustBrightness:1.f];
  // [hsbFilter adjustSaturation:1.1f];
  brightnessFilter = [[ISBrightnessFilter alloc] init];
  brightnessFilter.brightness = 0.f;

  [bilateralFilter addTarget:combinationFilter];
  [cannyEdgeFilter addTarget:combinationFilter];

  // [combinationFilter addTarget:hsbFilter];
  [combinationFilter addTarget:brightnessFilter];

  self.initialFilters =
      [NSArray arrayWithObjects:bilateralFilter, cannyEdgeFilter,
                                combinationFilter, nil];
...
#pragma mark public
- (void)setIntensity:(CGFloat)intensity {
  combinationFilter.intensity = intensity * 0.9;
}

- (void)setWhiteness:(CGFloat)whiteness {
  brightnessFilter.brightness = whiteness * 0.7;
}
                    ...
                    atIndex:(NSInteger)textureIndex;
{
  for (GPUImageOutput<GPUImageInput>* currentFilter in self.initialFilters) {
    ...
    [currentFilter setInputFramebuffer:newInputFramebuffer
