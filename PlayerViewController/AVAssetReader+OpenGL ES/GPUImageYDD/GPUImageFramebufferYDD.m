...
@interface GPUImageFramebufferYDD()
{
    GLuint framebuffer;
...
void dataProviderReleaseCallYDDback (void *info, const void *data, size_t size);
void dataProviderUnlockCallYDDback (void *info, const void *data, size_t size);

@implementation GPUImageFramebufferYDD
...
- (void)generateFramebuffer;
{
    runSynchronouslyOnVideoProcessingYddQueue(^{
        ...
        if ([GPUImageContextYDD supportsFastTextureUpload])
        {
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
            CVOpenGLESTextureCacheRef coreVideoTextureCache = [[GPUImageContextYDD sharedImageProcessingContext] coreVideoTextureCache];
...
- (void)destroyFramebuffer;
{
    runSynchronouslyOnVideoProcessingYddQueue(^{
        ...
        if ([GPUImageContextYDD supportsFastTextureUpload] && (!_missingFramebuffer))
        {
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
            if (renderTarget)
            {
...
- (void)unlock;
{
    ...
    {
        [[GPUImageContextYDD sharedFramebufferCache] returnFramebufferToCache:self];
    }
}
...
void dataProviderUnlockCallYDDback (void *info, const void *data, size_t size)
{
    GPUImageFramebufferYDD *framebuffer = (__bridge_transfer GPUImageFramebufferYDD*)info;
    
    [framebuffer restoreRenderTarget];
    [framebuffer unlock];
    [[GPUImageContextYDD sharedFramebufferCache] removeFramebufferFromActiveImageCaptureList:framebuffer];
}
...
- (CGImageRef)newCGImageFromFramebufferContents;
{
    ...
    runSynchronouslyOnVideoProcessingYddQueue(^{
        ...
        if ([GPUImageContextYDD supportsFastTextureUpload])
        {
            ...
            dataProvider = CGDataProviderCreateWithData((__bridge_retained void*)self, rawImagePixels, paddedBytesForImage, dataProviderUnlockCallYDDback);
            [[GPUImageContextYDD sharedFramebufferCache] addFramebufferToActiveImageCaptureList:self]; // In case the framebuffer is swapped out on the filter, need to have a strong reference to it somewhere for it to hang on while the image is in existence
#else
#endif
        }
        else
        {
            ...
            dataProvider = CGDataProviderCreateWithData(NULL, rawImagePixels, totalBytesForImage, dataProviderReleaseCallYDDback);
