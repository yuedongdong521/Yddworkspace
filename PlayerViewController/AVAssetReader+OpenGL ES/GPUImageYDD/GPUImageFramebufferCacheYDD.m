...
@interface GPUImageFramebufferCacheYDD()
{
...
@implementation GPUImageFramebufferCacheYDD
...
{
    __block GPUImageFramebufferYDD *framebufferFromCache = nil;
//    dispatch_sync(framebufferCacheQueue, ^{
    runSynchronouslyOnVideoProcessingYddQueue(^{
        ...
        if ([numberOfMatchingTexturesInCache integerValue] < 1)
        {
            ...
            framebufferFromCache = [[GPUImageFramebufferYDD alloc] initWithSize:framebufferSize textureOptions:textureOptions onlyTexture:onlyTexture];
        }
        else
        {
...
//    dispatch_async(framebufferCacheQueue, ^{
    runAsynchronouslyOnVideoProcessingYddQueue(^{
...
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
        CVOpenGLESTextureCacheFlush([[GPUImageContextYDD sharedImageProcessingContext] coreVideoTextureCache], 0);
#else
#endif
    });
}
