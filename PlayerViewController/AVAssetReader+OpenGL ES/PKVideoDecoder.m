...
@interface PKVideoDecoder () {
    GPUImageFramebufferYDD *outputFramebuffer;
    GLProgramYDD *yuvConversionProgram;
...
- (void)yuvConversionSetup {
    runSynchronouslyOnVideoProcessingYddQueue(^{
        ...
        _preferredConversion = kColorConversionYDD709;
        yuvConversionProgram = [[GPUImageContextYDD sharedImageProcessingContext] programForVertexShaderString:kGPUImageVertexShaderStr fragmentShaderString:kGPUImageYUVFullRangeConversionForLAFragmentShaderStr];
        ...
        [GPUImageContextYDD setActiveShaderProgram:yuvConversionProgram];
...
- (void)convertYUVToRGBOutput {
    [GPUImageContextYDD setActiveShaderProgram:yuvConversionProgram];
    outputFramebuffer = [[GPUImageContextYDD sharedFramebufferCache] fetchFramebufferForSize:CGSizeMake(imageBufferWidth, imageBufferHeight) onlyTexture:NO];
...
- (void)processMovieFrame:(CVPixelBufferRef)movieFrame withSampleTime:(CMTime)currentSampleTime {
    ...
    if (colorAttachments != NULL) {
        if(CFStringCompare(colorAttachments, kCVImageBufferYCbCrMatrix_ITU_R_601_4, 0) == kCFCompareEqualTo) {
            _preferredConversion = kColorConversion601FullRangeYDD;
        }
        else {
            _preferredConversion = kColorConversionYDD709;
        }
    ...
    if (CVPixelBufferGetPlaneCount(movieFrame) > 0) {// Check for YUV planar inputs to do RGB conversion
        ...
        // Y-plane
        glActiveTexture(GL_TEXTURE4);
        if ([GPUImageContextYDD deviceSupportsRedTextures]) {
            err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [[GPUImageContextYDD sharedImageProcessingContext] coreVideoTextureCache], movieFrame, NULL, GL_TEXTURE_2D, GL_LUMINANCE, bufferWidth, bufferHeight, GL_LUMINANCE, GL_UNSIGNED_BYTE, 0, &luminanceTextureRef);
        }
        else {
            err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [[GPUImageContextYDD sharedImageProcessingContext] coreVideoTextureCache], movieFrame, NULL, GL_TEXTURE_2D, GL_LUMINANCE, bufferWidth, bufferHeight, GL_LUMINANCE, GL_UNSIGNED_BYTE, 0, &luminanceTextureRef);
        }
        ...
        glActiveTexture(GL_TEXTURE5);
        if ([GPUImageContextYDD deviceSupportsRedTextures]) {
            err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [[GPUImageContextYDD sharedImageProcessingContext] coreVideoTextureCache], movieFrame, NULL, GL_TEXTURE_2D, GL_LUMINANCE_ALPHA, bufferWidth/2, bufferHeight/2, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, 1, &chrominanceTextureRef);
        }
        else {
            err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [[GPUImageContextYDD sharedImageProcessingContext] coreVideoTextureCache], movieFrame, NULL, GL_TEXTURE_2D, GL_LUMINANCE_ALPHA, bufferWidth/2, bufferHeight/2, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, 1, &chrominanceTextureRef);
        }
