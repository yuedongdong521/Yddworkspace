...
@property (nonatomic, strong) NSMutableDictionary <NSString *, NSArray *>*hostAddressDic;
...
- (NSMutableDictionary<NSString *, NSArray *> *)hostAddressDic
{
    if (!_hostAddressDic) {
        _hostAddressDic = [NSMutableDictionary dictionary];
    }
    return _hostAddressDic;
}
...
{
    if (!_dormainArray || _dormainArray.count == 0) return;

    _isRunning = YES;
    [_logInfo setString:@""];
    [self recordStepInfo:@"开始诊断..." curHost:nil];
...
//        [self recordOutIPInfo];
    ...
    if (_isRunning) {
        ...
        [self.dormainArray enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [self recordStepInfo:@"\n开始TCP连接测试..." curHost:obj];
            NSArray *hostAddress = [self.hostAddressDic objectForKey:obj];
            if (hostAddress.count > 0) {
                for (int i = 0; i < [hostAddress count]; i++) {
                    [_netConnect runWithHostAddress:[hostAddress objectAtIndex:i] port:80];
                }
            } else {
               [self recordStepInfo:@"DNS解析失败，主机地址不可达" curHost:obj];
            }
        }];
    ...
    if (_isRunning) {
        ...
        if (_traceRouter) {
            [_dormainArray enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
{
    //输出应用版本信息和用户ID
    [self recordStepInfo:[NSString stringWithFormat:@"应用code: %@", _appCode] curHost:nil];
    ...
    [self recordStepInfo:[NSString stringWithFormat:@"应用名称: %@", _appName] curHost:nil];

    if (!_appVersion || [_appVersion isEqualToString:@""]) {
        _appVersion = [dicBundle objectForKey:@"CFBundleShortVersionString"];
    }
    [self recordStepInfo:[NSString stringWithFormat:@"应用版本: %@", _appVersion] curHost:nil];
    [self recordStepInfo:[NSString stringWithFormat:@"用户id: %@", _UID] curHost:nil];

    //输出机器信息
    UIDevice *device = [UIDevice currentDevice];
    [self recordStepInfo:[NSString stringWithFormat:@"机器类型: %@", [device systemName]] curHost:nil];
    [self recordStepInfo:[NSString stringWithFormat:@"系统版本: %@", [device systemVersion]] curHost:nil];
    if (!_deviceID || [_deviceID isEqualToString:@""]) {
        _deviceID = [self uniqueAppInstanceIdentifier];
    }
    [self recordStepInfo:[NSString stringWithFormat:@"机器ID: %@", _deviceID] curHost:nil];
    ...
    [self recordStepInfo:[NSString stringWithFormat:@"运营商: %@", _carrierName] curHost:nil];
    [self recordStepInfo:[NSString stringWithFormat:@"ISOCountryCode: %@", _ISOCountryCode] curHost:nil];
    [self recordStepInfo:[NSString stringWithFormat:@"MobileCountryCode: %@", _MobileCountryCode] curHost:nil];
    [self recordStepInfo:[NSString stringWithFormat:@"MobileNetworkCode: %@", _MobileNetCode] curHost:nil];
}
...
//    [self recordStepInfo:[NSString stringWithFormat:@"\n\n诊断域名 %@...\n", _dormain]];
    ...
    if (_curNetType == 0) {
        [self recordStepInfo:[NSString stringWithFormat:@"当前是否联网: 未联网"] curHost:nil];
    } else {
        [self recordStepInfo:[NSString stringWithFormat:@"当前是否联网: 已联网"] curHost:nil];
        if (_curNetType > 0 && _curNetType < 6) {
            [self
                recordStepInfo:[NSString stringWithFormat:@"当前联网类型: %@",
                                                          [typeArr objectAtIndex:_curNetType - 1]] curHost:nil];
        }
    }

    //本地ip信息
    _localIp = [LDNetGetAddress deviceIPAdress];
    [self recordStepInfo:[NSString stringWithFormat:@"当前本机IP: %@", _localIp] curHost:nil];

    if (_curNetType == NETWORK_TYPE_WIFI) {
        _gatewayIp = [LDNetGetAddress getGatewayIPAddress];
        [self recordStepInfo:[NSString stringWithFormat:@"本地网关: %@", _gatewayIp] curHost:nil];
    } else {
        _gatewayIp = @"";
    }
    ...
    [self recordStepInfo:[NSString stringWithFormat:@"本地DNS: %@",
                                                    [_dnsServers componentsJoinedByString:@", "]] curHost:nil];
    
    
    [self.dormainArray enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
         [self recordStepInfo:[NSString stringWithFormat:@"远端域名: %@", obj] curHost:nil];
         long time_start = [LDNetTimer getMicroSeconds];
         NSArray *hostDNSArr = [LDNetGetAddress getDNSsWithDormain:obj];
         long time_duration = [LDNetTimer computeDurationSince:time_start] / 1000;
        if (hostDNSArr.count > 0) {
            NSString *mesg = [NSString stringWithFormat:@"DNS解析结果: %@ (%ldms)",
                              [hostDNSArr componentsJoinedByString:@", "],
                              time_duration];
            [self recordStepInfo:mesg curHost:obj];
            [self.hostAddressDic setValue:hostDNSArr forKey:obj];
        } else {
            [self recordStepInfo:[NSString stringWithFormat:@"DNS解析结果: 解析失败"] curHost:obj];
        }
    }];
...
- (void)recordOutIPInfo
{
    ...
    [self recordStepInfo:response curHost:kCheckOutIPURL];
}
...
- (void)pingDialogsis:(BOOL)pingLocal
{
    ...
    for (int i = 0; i < [pingAdd count]; i++) {
        [self recordStepInfo:[NSString stringWithFormat:@"ping: %@ %@ ...",
                                                        ...
                                                        [pingAdd objectAtIndex:i]] curHost:nil];
...
{
    [self recordStepInfo:pingLog curHost:nil];
}
...
{
    [self recordStepInfo:routeLog curHost:nil];
}
...
{
    [self recordStepInfo:socketLog curHost:nil];
}
 ...
 */
- (void)recordStepInfo:(NSString *)stepInfo curHost:(NSString *)curHost
{
    if (stepInfo == nil) stepInfo = @"";
    if (curHost == nil) curHost = @"";
    
    
    if (curHost.length > 0) {
        stepInfo = [NSString stringWithFormat:@"%@ : %@", curHost, stepInfo];
    }
