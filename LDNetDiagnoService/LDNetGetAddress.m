...
+ (void)getPublicNetworkIpCompleted:(void(^)(KXUserIpModel *model))completed
{
    ...
    /// https://icanhazip.com
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"http://ip-api.com/json"] cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:10];
    NSURLSessionTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (data) {
            KXUserIpModel *model = [[KXUserIpModel alloc] initWithDic:[NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:nil]];
            if (completed) {
                completed(model);
            }
        } else {
...
#pragma clang diagnostic pop
    ...
    } else if ([[app valueForKeyPath:@"_statusBar"] isKindOfClass:NSClassFromString(@"UIStatusBar_Modern")]) {  //iOS11
